foreach(required_variable
        OBJDUMP UEFI_IMAGE BIOS_STAGE1 BIOS_STAGE2 BIOS_STAGE2_PE)
...
# The combined raw image is assembled by a host tool, so a cross build produces
# every binary except that one and simply omits it here.
set(GOOSEBOOT_HAVE_BIOS_IMAGE FALSE)
if(DEFINED BIOS_IMAGE AND NOT "${BIOS_IMAGE}" STREQUAL "")
    if(NOT EXISTS "${BIOS_IMAGE}")
        message(FATAL_ERROR "Missing firmware verification input: ${BIOS_IMAGE}")
    endif()
    set(GOOSEBOOT_HAVE_BIOS_IMAGE TRUE)
endif()
...
if(DEFINED BIOS_CDSTUB AND NOT "${BIOS_CDSTUB}" STREQUAL "")
    if(NOT EXISTS "${BIOS_CDSTUB}")
        message(FATAL_ERROR "Missing firmware verification input: ${BIOS_CDSTUB}")
    endif()
    file(SIZE "${BIOS_CDSTUB}" cdstub_size)
    if(NOT cdstub_size EQUAL 512)
        message(FATAL_ERROR "BIOS CD stub must be exactly 512 bytes, got ${cdstub_size}")
    endif()
    file(READ "${BIOS_CDSTUB}" cdstub_signature OFFSET 510 LIMIT 2 HEX)
    string(TOLOWER "${cdstub_signature}" cdstub_signature)
    if(NOT cdstub_signature STREQUAL "55aa")
        message(FATAL_ERROR "BIOS CD stub boot signature is missing")
    endif()
    # The stub exists so an optical boot needs no disk service at all: assert it
    # holds no INT 13h opcode (CD 13) anywhere in the sector.
    file(READ "${BIOS_CDSTUB}" cdstub_hex HEX)
    if(cdstub_hex MATCHES "cd13")
        message(FATAL_ERROR "BIOS CD stub must not contain an INT 13h call")
    endif()
endif()

file(READ "${BIOS_STAGE1}" stage1_signature OFFSET 510 LIMIT 2 HEX)
string(TOLOWER "${stage1_signature}" stage1_signature)
if(NOT stage1_signature STREQUAL "55aa")
...
if(GOOSEBOOT_HAVE_BIOS_IMAGE)
    ...
    if(NOT image_signature STREQUAL "55aa")
        message(FATAL_ERROR "BIOS boot signature is missing")
    endif()
...
message(STATUS
    "Firmware verified: UEFI=${uefi_size} bytes x64/DIR64, BIOS=512+65024 bytes, "
    "raw image=${GOOSEBOOT_HAVE_BIOS_IMAGE}, no UEFI DLL imports")
